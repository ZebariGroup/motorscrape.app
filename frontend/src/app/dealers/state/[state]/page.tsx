...
import { ALL_STATES, POPULAR_MAKES, stateBySlug } from "@/lib/dealerDirectory";
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = `Car Dealerships in ${stateEntry.name} | Motorscrape`;
  const description = `Browse the complete directory of car dealerships in ${stateEntry.name}. Find dealer ratings, phone numbers, hours, brands carried, and live inventory.`;

  return {
    title,
    description,
    alternates: { canonical: `/dealers/state/${stateSlug}` },
...
export default async function StateDirectoryPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    "@type": "ItemList",
    name: `Car Dealerships in ${stateEntry.name}`,
    description: `Directory of car dealerships in ${stateEntry.name}`,
    url: `https://www.motorscrape.com/dealers/state/${stateSlug}`,
    numberOfItems: total,
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 3, name: stateEntry.name, item: `https://www.motorscrape.com/dealers/state/${stateSlug}` },
    ],
  };
      ...
      <div className="bg-zinc-950 border-b border-zinc-800">
        <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14">
          ...
          <nav className="mb-4 flex items-center gap-1.5 text-xs text-zinc-500" aria-label="Breadcrumb">
            ...
            <span aria-hidden>/</span>
            <span className="text-zinc-300">{stateEntry.name}</span>
          </nav>

          <h1 className="text-3xl font-extrabold tracking-tight text-white sm:text-4xl">
            Car Dealerships in {stateEntry.name}
          </h1>
          <p className="mt-2 text-zinc-400 text-sm sm:text-base">
            {total > 0
              ? `${total.toLocaleString()} dealer${total !== 1 ? "s" : ""} in ${stateEntry.name} — ratings, phone numbers, hours, and live inventory`
              : `Dealerships in ${stateEntry.name} appear as inventory is discovered`}
          </p>

          <div className="mt-5 flex flex-wrap gap-2">
            <Link
              href={`/dealers?state=${stateEntry.abbr}`}
            ...
            >
              Filter &amp; Sort {stateEntry.name} Dealers
            </Link>
            <Link
              href="/dealers"
      ...
      <main className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6">
        {dealers.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-zinc-300 bg-zinc-50 p-12 text-center dark:border-zinc-700 dark:bg-zinc-900/40">
            <p className="text-sm font-semibold text-zinc-700 dark:text-zinc-300">No dealers indexed yet for {stateEntry.name}</p>
          ...
          <>
            <div className="mb-4 flex items-center justify-between">
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                ...
                dealers in {stateEntry.name}
              </p>
              {total > LIMIT && (
                <Link href={`/dealers?state=${stateEntry.abbr}`} className="text-xs text-emerald-600 hover:text-emerald-700 dark:text-emerald-400 font-medium">
                  View all →
                </Link>
              )}
            </div>
            ...
            {total > LIMIT && (
              <div className="mt-8 text-center">
                ...
                >
                  View All {total.toLocaleString()} {stateEntry.name} Dealers
        ...
        <section className="mt-16 border-t border-zinc-200 pt-10 dark:border-zinc-800">
          <h2 className="mb-1 text-lg font-bold text-zinc-900 dark:text-zinc-100">
            Browse {stateEntry.name} Dealers by Brand
          </h2>
          <p className="mb-5 text-sm text-zinc-500 dark:text-zinc-400">
            Find dealers carrying specific makes in {stateEntry.name}.
          </p>
          <div className="flex flex-wrap gap-2">
            {POPULAR_MAKES.map((m) => (
              <Link
                ...
                className="rounded-full border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium text-zinc-700 hover:border-sky-300 hover:text-sky-700 hover:bg-sky-50 transition-colors dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300 dark:hover:border-sky-700 dark:hover:text-sky-300"
              >
                {m.name}
              </Link>
            ))}
        ...
        <section className="mt-12 border-t border-zinc-200 pt-10 dark:border-zinc-800">
          ...
          <p className="mb-5 text-sm text-zinc-500 dark:text-zinc-400">
            Explore dealership directories across the country.
          </p>
